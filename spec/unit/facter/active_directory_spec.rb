...
describe 'active_directory' do
  ...
  context 'no commands exist' do
    it 'should be nil' do
      Facter::Core::Execution.stubs(:which).with('realm').returns(nil)
      Facter::Core::Execution.stubs(:which).with('adcli').returns(nil)
  ...
  context 'adcli but no realm command' do
    it 'have a fact that matches the expected' do
      ...
      Facter::Core::Execution.stubs(:which).with('realm').returns(nil)
      Facter::Core::Execution.stubs(:which).with('adcli').returns('/usr/sbin/adcli')
      Facter::Core::Execution.stubs(:exec).with('/usr/sbin/adcli info test.case', timeout: 15).returns(adcli_file)
  ...
  context 'both commands exist' do
    it 'have a fact that matches the expected' do
      ...
      Facter::Core::Execution.stubs(:which).with('realm').returns('/usr/sbin/realm')
      Facter::Core::Execution.stubs(:which).with('adcli').returns('/usr/sbin/adcli')
      Facter::Core::Execution.stubs(:exec).with('/usr/sbin/adcli info test.case', timeout: 15).returns(adcli_file)
      Facter::Core::Execution.stubs(:exec).with('/usr/sbin/realm list', timeout: 15).returns(realm_file)
